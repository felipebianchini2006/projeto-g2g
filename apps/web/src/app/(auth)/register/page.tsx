...
export default function Page() {
  ...
  const nextPath = '/';
  const googleStartUrl = `/api/auth/google/start?next=${encodeURIComponent(
    nextPath,
      ...
      <Card className="relative w-full max-w-[420px] rounded-[32px] border-none bg-white shadow-[0_20px_40px_-10px_rgba(0,0,0,0.1)]">
        ...
        <CardContent className="grid gap-5 px-8 pb-10">
          ...
          <form className="grid gap-5" onSubmit={handleSubmit}>
            ...
            <div className="space-y-1.5">
              <label htmlFor="role" className="ml-1 text-xs font-bold uppercase tracking-wide text-slate-500">
                Perfil
              </label>
              <div className="relative">
                {/* Trick: put transparent icon overlay to match style, but Select is slightly different */}
                <User className="absolute left-4 top-1/2 h-5 w-5 -translate-y-1/2 text-slate-400 z-10 pointer-events-none" />
                <Select
                  id="role"
                  className="h-12 rounded-xl border-slate-200 bg-slate-50 pl-12 font-medium text-slate-900 transition-all focus:bg-white focus:ring-4 focus:ring-meow-primary/10"
                  value={formData.role}
              ...
              {errors.role && <p className="ml-1 text-xs font-semibold text-red-500">{errors.role}</p>}
            </div>
            ...
            >
              {isSubmitting ? 'Gerar conta' : 'Criar conta'}
            </Button>
          </form>
